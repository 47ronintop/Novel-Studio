...
| TD-015 | `TESTING.md` 第5/7/8节                                   | Schema valid/invalid fixtures 已建立，但 CI 配置、coverage threshold 和 Playwright smoke 仍未实现                      | 自动门禁尚未进入远端 CI                                 | Phase 7 M3-M4 后补 CI、coverage、Playwright smoke 任务                | Open   |
...
| TD-021 | `docs/packaging/m10-beta-packaging.md` | Vite renderer bundling and electron-builder config are in place, but `npm run package:dir` exceeded the local 180 second command timeout on the current Windows/Node 20.20.2 workstation. | Package preflight is repeatable, but unpacked/installer artifact production still needs a stable packaging host. | Verify `package:dir` on a packaging host, then add installer targets and signing only after unpacked packaging is stable. | Open |
...
| TD-020 | 2026-07-03  | M3 已创建 Repository Core：项目读取校验、原子写入、history snapshot、recovery record、cache clear guard，以及对应 10 个 focused tests。                         |
