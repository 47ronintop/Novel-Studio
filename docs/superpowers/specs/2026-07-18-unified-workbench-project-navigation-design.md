...
Windows/Linux 窗口最上方的 Electron 原生“文件”菜单是应用级项目生命周期入口；macOS 使用同一 Application Menu template。不得在自定义 Title Bar 再复制一个“文件”按钮。

- “文件”菜单至少包含“新建创作项目…”、“打开创作项目…”、“打开工程文件夹…”和现有“关闭窗口”。
- 菜单只发出语义命令；`menu.ts` 不直接访问 Repository、文件系统或项目 Session。正确路径是 Native Menu -> focused Renderer command -> `WorkspaceNavigation` -> 现有 Application/IPC -> Repository。
- “新建创作项目…”打开中央创建表面或模态编辑器；项目标题、文件夹名称和父目录选择不得塞进原生下拉菜单。
- 无工作区欢迎页可以重复提供新建/打开主动作；Navigator 不再常驻项目创建表单，也不再拥有“打开项目 / 创建项目 / 打开工程目录”按钮。
- 未实现最近项目仓库前，不显示失效的“最近打开”子菜单。没有统一活动文档命令前，也不伪造“全部保存”或“关闭工作区”。
- 菜单命令失败时保留旧 Workspace Context、Editor、Agent Conversation 和草稿，并在创建表面或中央欢迎页附近显示 Unified Error。
- 菜单项提供平台惯用 accelerator、可访问名称和键盘焦点恢复；同一语义也可由 Command Center 调用，但不得复制业务实现。
...
入口来自原生“文件 > 新建创作项目…”或无工作区中央欢迎页。创建表面是一次性任务界面，不属于项目 Navigator。
...
- 除用户显式折叠、专注模式和窄窗口响应式折叠外，完整 `AgentConversationView` 外壳始终存在；`workspaceContext.kind === "none"` 不得用“未绑定工作区”占位页替换整个 Agent Surface。
- Settings、Search、Timeline、Story Bible、Studio 和中央 Review 都在中央 Editor Area 打开，不得通过替换整个 Workbench Shell 使右侧 Agent 或左侧布局所有权消失。
- 无工作区时仍显示 Agent 标题栏、Conversation 空态和 `aria-label="会话输入区"` 的 Composer 结构，但发送、新建会话、历史和运行相关动作禁用，并提示先新建或打开工作区。
- 无工作区表面只是 UI projection：不得实例化 Conversation bridge、调用 Agent API、创建 app-local/global Conversation、Run 或 `prj_minimal_chapter` 一类虚拟 ID。
...
`WorkbenchMode` 与 `AgentContextMode` 是相关但不同的状态，不得合并为一个 enum：

- 工作台决定 Navigator、中央编辑表面和状态栏；Agent 上下文决定单次 Run 的系统指导、可引用来源和工具边界。
- 用户界面将 `writing/general_file` 标为“写作上下文 / 文件上下文”，不再使用容易与“创作工作台 / 工程工作台”混淆的裸标签“写作 / 通用文件”。
- 创作项目默认使用写作上下文，并保留文件上下文作为显式的单次任务选择；普通工程工作区只提供文件上下文。
- 工作台切换不得重写 active run、已持久化 Plan、Conversation turn 或已有 Context Snapshot。工程工作区的非法 `writing` draft 仍通过既有 Application draft mutation 归一化，不在 UI 层静默伪造状态。
- 无工作区 Composer 可以投影当前工作台建议的上下文标签，但所有上下文、模型和发送动作保持禁用，不持久化 draft。
...
- Settings 作为中央编辑/模态表面打开，保留 Title Bar、Activity Bar、Navigator、右侧 Agent 和 Status Bar 的 Shell 所有权；关闭后恢复此前中央表面与焦点。
- Agent 模型、推理、模式、审批和引用必须与输入框形成同一 Composer 组。
- 右侧 Agent 采用成熟 IDE 的 Secondary Side Bar 语义：位置稳定、可折叠、项目切换前不闪烁为另一套聊天表面；Editor 始终获得剩余最大空间。
- 原生“文件”菜单与中央欢迎页负责项目生命周期入口；Navigator 只投影当前内容。创作 Navigator 的项目标题旁溢出菜单只放当前项目操作，不放全局打开/创建表单。
...
- Electron 安装包只保留当前正式支持的 `zh-CN` 与 `en-US` Chromium locale pack；这不等同于应用界面国际化，也不得因此增加没有翻译资源支撑的语言选择器。
...
- 未建立应用级翻译目录、回退规则和 app-local locale 偏好前，不增加虚假的界面语言切换。
...
- 顶部工作台选择清晰可见。
- Electron 原生“文件”菜单提供新建创作项目、打开创作项目和打开工程文件夹；左侧 Navigator 不再常驻这些入口或创建表单。
- Settings 在中央 Editor Area 打开，周围 Navigator、Agent、Status Bar、Conversation 和 run state 保持挂载。
...
- 无工作区时同一个 Agent Surface 和 Composer 结构仍可见但不可发送，且没有 Conversation/Run API 调用或虚拟 ID。
- Plan/Act、写作上下文/文件上下文、审批、模型、推理和引用均在 Composer 内；工作台标签不作为第二套上下文按钮重复出现。
...
- 打包目录只包含 `zh-CN.pak` 与 `en-US.pak` 两个 Electron locale 文件。
...
完成后，Novel Studio 使用一个稳定 Shell 服务创作项目与普通工程。原生“文件”菜单和中央欢迎页承担项目生命周期入口，Navigator 只管理当前内容。创作项目默认直接显示章节，故事资料通过稳定页签进入；工程工作台提供受控文件树和代码编辑；创建创作项目不会污染父目录；打开普通工程不会初始化创作结构。右侧始终投影唯一 Agent Surface：绑定工作区后加载项目级会话，无工作区时保留完整但禁用的 Composer，且不创建虚拟 Conversation/Run。会话、Plan/Act、审批、模型、推理和引用集中在 Composer，Plan/Diff/Change Set 在中央审阅。所有导航来源统一编排，Change Set、Version Group、journal、recovery、undo、草稿和 Conversation 在工作台切换中保持原义。
