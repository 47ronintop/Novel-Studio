...
除上述空项目入口外，导航状态、Agent draft 持久化和运行时权限边界均不改造。
...
3. 接入现有激活/最后章节加载链路。
4. 增加空项目“开始构思”按钮、会话复用、Composer 预填/聚焦和草稿保护。
5. 补齐失败清理、安全边界、接入反馈和端到端定向测试。
6. 另行评估原地初始化、导航改造和 Agent 审稿模式，不在本 MVP 中顺带实现。
