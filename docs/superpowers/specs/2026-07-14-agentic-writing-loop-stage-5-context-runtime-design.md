...
- 不把“本次运行自动写入”升级为项目级永久默认；仍然只能由用户对当前 execution run 明确授权。
...
- “写入前确认”不是低权限模式，而是默认审批方式。
- “本次运行自动写入”不是扩大工具或路径权限，而是当前 run 的显式预授权。
...
手动压缩入口位于 Agent composer 的上下文 meter 旁，按钮名为“压缩上下文”。点击后直接进入短暂的 `context_compacting` 状态，不再弹出重复确认；用户可先通过“查看来源”检查将参与压缩的范围。压缩不与普通模型轮次并行，也不产生文件副作用。
...
当模型窗口未知、usage 仅为估算、引用过期或存在 dirty source 时，显示对应状态。点击“查看来源”打开只读面板，列出每个 source 的类型、范围、checksum、token 占用、dirty/stale 状态和排除原因。
...
- 可提案的文件类型/目录；
- 是否允许生成 Change Set；
- 写入策略和 approval source；
- 明确列出不可用能力：Shell、Git、网络、删除、移动、重命名、创建目录。

摘要是事实 DTO，不由模型生成。它可折叠，但不能只放在设置页；用户在发送、等待审批和自动写入期间都能打开查看。

### 6.2 审批策略

执行模式提供两个互斥选项：

- `写入前询问`：每个 checkpoint 生成 Change Set，目标文件不变，用户在 Diff Review 中确认。
- `本次运行自动写入`：用户必须先看到风险说明并勾选确认；每个 Change Set 仍经过同一 diff、校验、快照、journal、Version Group 和 undo 管线。

自动写入只绑定当前 run，应用重启、创建新 run 或切换 Conversation 后恢复默认人工确认。模型文本、工具参数、文件正文、历史记录和项目偏好都不能改变它。
...
3. **诊断抽屉**：可复制 error ID、run ID、provider/model、事件序号、脱敏细节和重试/恢复记录。
...
Agent composer 的上下文 meter 只显示当前输入预算；run 详情显示本轮和累计 token。两者使用同一 Usage DTO，避免设置页和 run 面板出现两套数字。
...
run 错误保存到自己的系统目录；preflight 错误保存到项目的 `history/agent-diagnostics` 并受数量/保留上限约束。snapshot 增加 `activeErrorId` 和 `recoveryState(none/retryable/awaiting_context_refresh/recovery_review/terminal)`。重载后诊断抽屉必须展示相同 error ID。transaction partial failure 继续以 recovery journal 为事实源，error record 只引用它，不复制或替代 journal。
...
流式 partial usage 只更新当前 round 内存聚合和 `usage_updated` UI 事件；round 结束时才持久化最终记录。相同 usage ID 重放返回首次记录，不重复计费。记录不包含 API key、提示词、文件正文或隐藏推理。

### 8.9 `AgentRunSnapshot` v1.1

Stage 5 新 run 写 `AgentRunSnapshot schemaVersion: 1.1`，在现有字段上增加 `permissionSummaryId/checksum`、`contextBudgetSnapshotId`、`activeCompactionId`、`planExecutionId/revision`、`activeErrorId`、`recoveryState` 和累计 usage 摘要，并扩展 `status` 支持 `context_compacting` 与 `awaiting_plan_revision`。
...
| `usage_updated` | roundId、partial token、usageStatus | 不持久化最终 usage 前只更新 UI |
...
- 自动压缩、重试、恢复、计划修订和自动写入都必须具备 command idempotency。
...
- 自动写入只能由当前 run 的用户命令授权，模型输出和文件内容不能改变策略。
...
- run 内 usage 与设置页聚合使用同一 DTO；每日图表按本地时区稳定分桶。
...
- 真实 Electron Playwright 覆盖错误诊断抽屉、复制 error ID、重试/恢复、上下文 meter 和设置页用量图。
