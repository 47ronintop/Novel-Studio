...
下文的细粒度 Task 用于锁定合同、文件范围和验收点，实际实施不按每个 Task 单独启动一轮开发、全量测试或提交。默认合并为六个执行批次；批次内共享实现、UI、恢复和测试一起完成，批次末统一执行门禁。

### 六个执行批次

| 批次          | 合并范围                                 | 主要工作                                                                                             | 依赖与完成条件                                                          |
| ------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 1. 公共基座   | Phase 0.1-0.4                            | 一次完成 canonical catalog、严格 Schema、provider 映射、能力状态、执行端口和 Run Snapshot/Event v1.2 | 无前置批次；统一通过 Phase 0 门禁后，批次 2/3/4 可并行启动              |
| 2. 本地工具   | Phase A + Phase B                        | 搜索/引用与文件生命周期并行开发，最后统一接入 registry、run session、UI、事务和恢复                  | 依赖批次 1；A/B 各自安全验收保留，定向测试合并执行                      |
| 3. 外部读取   | Phase D + Phase E.3 + 远程来源部分 E.4   | 先完成网络策略、设置和数据外发审批，再接远程 MCP 及其来源管理                                        | 依赖批次 1；远程 MCP 必须等 Phase D 网络门禁通过                        |
| 4. 任务与 Git | Phase C.0-C.4                            | 并行推进原生 host/probe、Task Catalog/审批和 Git adapter，最后完成任务与 Git 纵向集成                | 依赖批次 1；C.0 仍是不可绕过的资格门，未通过时相关工具保持隐藏          |
| 5. 本地扩展   | Phase E.1 + Phase E.2 + 本地来源部分 E.4 | 插件和本地 stdio MCP 并行，共用已验证的原生 host、动态目录、审批和来源 UI                            | 依赖批次 4 的 C.0；插件与本地 MCP 分别通过自身 profile 门禁即可独立交付 |
| 6. 发布收口   | Phase F                                  | 只做跨批次回归、旧数据兼容、打包产物验证、人工验收和发布状态同步                                     | 等待准备发布的批次通过各自门禁；不在本批次首次实现产品功能              |

批次 2、3、4 在批次 1 完成后可并行。批次 5 只依赖批次 4 的 C.0，不要求任务/Git 的所有产品能力先发布；批次 6 只汇总本次准备发布的能力，未完成批次继续保持 flag 关闭和 `unavailable`，不阻塞已独立达标的批次。

### 子代理与模型选择

执行时根据风险、共享程度和任务边界选择模型，不为追求并发把同一个共享文件拆给多个代理。默认每个批次使用 2-4 个并行子代理；只有文件所有权清晰、任务真正独立且并行收益大于集成成本时才增加。

| 角色/模型       | 适用任务                           | 默认职责                                                                                                                                     |
| --------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 主代理          | 跨模块合同和共享热点               | 持有批次计划、`tool-registry.ts`、`agent-run-session.ts`、IPC 合同等共享文件；协调接口、集成子代理结果、处理冲突、执行批次门禁并给出最终结论 |
| `gpt-5.6-terra` | 边界明确、可独立验证的普通工程任务 | Schema/DTO、搜索 Repository、设置页和普通 UI、来源管理、测试 fixture、窄范围 adapter 及相应单元测试                                          |
| `gpt-5.6-sol`   | 高风险、共享或需要系统推理的任务   | 权限/事件合同、持久化迁移、Change Set 事务、no-follow 文件操作、原生沙箱、SSRF/固定 dialer、Git 信任边界、跨模块集成和高风险审查             |

模型分配遵循以下规则：

1. 清晰、局部、可逆且有直接测试入口的任务优先交给 `gpt-5.6-terra`，减少关键路径等待。
2. 涉及安全边界、恢复语义、跨模块公共合同、原生代码或高代价回归的任务使用 `gpt-5.6-sol`。
3. 同一批次可由多个 `gpt-5.6-terra` 子代理并行处理独立模块，并由一个 `gpt-5.6-sol` 或主代理负责关键合同/集成；不为形式统一强制所有任务使用同一模型。
4. 子代理开始前必须声明独占文件/目录、依赖接口和预期测试；共享热点由主代理修改，其他代理只通过约定接口对接。
5. 子代理交付时必须报告修改文件、已运行测试、失败/跳过项和残余风险；主代理核对 diff 后再集成，不直接把子代理结论当成门禁证据。
6. 子代理不独立执行最终提交；需要提交时由主代理在批次门禁通过后统一暂存和提交，避免共享工作区出现交叉提交。

### 合并后的测试节奏

批次化只减少重复执行，不降低设计文件和各 Phase 已列出的验收标准：

1. 子代理开发期间只运行自己修改模块的最窄相关测试；非平凡逻辑和回归优先先写失败测试，文案、样式、配置和生成文件不机械执行测试先行仪式。
2. 同一批次内不在每个小 Task 后重复运行全仓 `typecheck`、`lint`、build 或 E2E。主代理在接口合并后统一运行该批次所有定向测试，再各运行一次必要的类型检查和 lint。
3. 批次 2 将 Phase A/B 测试集合并执行；批次 3 合并网络和远程 MCP 测试；批次 4 合并 C.0-C.4 的 Rust、沙箱、任务和 Git 门禁；批次 5 合并插件与本地 MCP 的打包验证。
4. `npm audit` 只在依赖或锁文件变化的批次以及最终发布批次运行；无依赖变化时不重复运行。
5. 完整 build/E2E/package 验证集中在涉及打包产物的批次 4/5 和最终批次 6；其他批次只在其合同或运行时变化确实需要时提前执行。
6. C.0 黑盒负对照、destructive file race/恢复、Phase D SSRF/数据外发、插件/本地 MCP 沙箱、远程 MCP endpoint/`outcome_unknown` 和最终打包验证是独立安全证据，不能因批次合并而删除或用普通单元测试代替。
7. 任一批次门禁失败时先修复该批次，不启动依赖它的新工作；与失败边界无依赖的已启动批次可以继续。
