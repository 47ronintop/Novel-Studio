...
| 自动保存与崩溃恢复 UI                   | `PROJECT_CONSTITUTION.md` 第5节，`UI_GUIDELINES.md` 第5.2节                     | M38 已写入 recovery record；M49 已补预览/应用/丢弃；M51 已补 clean 隐藏和 file-ref typed error                                                | 仍缺 file-ref dereference policy、恢复记录归档/清理策略和 stale-lock recovery UI                         | M51 完成，后续 Recovery Archive    |
...
| 插件系统                                | `PROJECT_CONSTITUTION.md` 第10节，`PLUGIN_SYSTEM.md`                            | Manifest/registry/permission engine、manifest 摘要读取、启停管理和权限详情 UI 已有；M54 已补 Plugin Runtime RFC                               | 仍缺 host-command runtime、workflow-step adapter、Command Palette contribution 和 sandboxed-code RFC     | M54 完成，后续 Plugin Runtime 实现 |
...
| Workflow Branch                         | `DATA_SCHEMA.md` / `WORKFLOW_ENGINE.md`                                         | M45 已补 `choose-branch` action、`chooseWorkflowBranch()`、branch schema/fixture 契约；M53 已补 workflow rail；M56 已补 Workflow Designer RFC | 仍缺 graph projection、validator、node inspector、Agent branch decision 展示和插件 workflow contribution | M56 完成，后续 Designer 实现       |
| 编辑器体验                              | `UI_GUIDELINES.md` 第5.2节，TECH_DEBT TD-033                                    | M46 已补文档指标、large-document mode、gutter 上限、diff 摘要和快捷键冲突矩阵；M52 已补 editor runtime 状态条；M55 已补 Editor Runtime RFC    | 仍缺 textarea adapter 抽取、CodeMirror adapter、selection metadata 和 visual diff runtime                | M55 完成，后续 Editor Runtime 实现 |
...
| 首次使用引导                            | ROADMAP M27 / M48                                                               | M48 已补工作区快速开始、示例项目入口、创建/打开项目引导和第一章行动按钮；M50 已持久化 dismissed 状态                                          | 仍缺更完整的项目模板和新手任务进度                                                                       | M50 完成，后续 Templates           |
...
3. M38/M49/M51 Autosave Recovery：自动保存恢复记录、恢复提示、预览、应用、丢弃和基础加固。
...
10. M52/M53：Editor Runtime 与 Workflow UX 已完成首轮可见 runtime/rail 补齐。
11. M54-M56：Plugin Runtime、Editor Runtime Engine 和 Workflow Designer RFC 已完成，后续进入实现批次。
12. M57+：Plugin host commands、plugin workflow-step adapter、editor runtime adapter extraction 和 workflow graph projection。
