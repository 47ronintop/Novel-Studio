...
| 插件系统                                | `PROJECT_CONSTITUTION.md` 第10节，`PLUGIN_SYSTEM.md`                            | Manifest/registry/permission engine 和只读 UI 已有          | 缺安装/启停/manifest 读取、沙箱执行、Workflow 接入 UI                                           | M42 Plugin Management   |
| LLM Provider 覆盖                       | `PROJECT_CONSTITUTION.md` 第3节，TECH_DEBT TD-018                               | Mock、OpenAI-compatible、OpenAI、Ollama 方向已部分覆盖      | 缺 Anthropic、Gemini、OpenRouter、DeepSeek、智谱、通义千问、LM Studio、vLLM fixtures 和 UI 验证 | M43 Provider Matrix     |
| OpenAI-compatible Streaming             | `PROJECT_CONSTITUTION.md` 第3节                                                 | 非流式路径可用；streaming 明确未实现                        | 与“流式与非流式统一抽象”仍有差距                                                                | M44 Streaming Adapter   |
| Workflow Branch                         | `DATA_SCHEMA.md` / `WORKFLOW_ENGINE.md`                                         | 基础状态机可用；branch step 明确未实现                      | 工作流表达能力不足，复杂流程需要绕开 branch                                                     | M45 Workflow Branch     |
...
7. M42 Plugin Management：插件 manifest 读取、启停、权限详情和安装入口。
