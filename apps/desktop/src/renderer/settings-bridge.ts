import type { ModelProfile, NovelStudioApi } from "@novel-studio/application";
import type { ModelSettingsDraft, ModelSettingsPanelProps } from "@novel-studio/ui";
...
export function createSettingsBridge(
  ...
  return {
    getProps: () => toProps(),
    async load() {
      const result = await api.settings.listModelProfiles();
      if (!result.ok) {
