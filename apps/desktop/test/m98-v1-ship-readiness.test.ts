...
describe("M98 V1 ship readiness", () => {
  test("records the v1 ship decision, evidence, limits, and reading aloud scope", async () => {
    ...
    expect(document).toContain("V1 ship decision: GO");
    ...
    expect(document).toContain("Known limitations do not block the core writing loop.");
  ...
  test("release check validates M98 readiness without publishing", () => {
    ...
    expect(result.stdout).toContain("V1 ship readiness gate passed");
