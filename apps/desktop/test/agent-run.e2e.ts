...
test("stops a live Agent run through the real Electron IPC path", async () => {
  ...
  try {
    ...
    await expect(page.locator(".ns-agent-status")).toHaveText("已停止");
    await expect(page.locator(".ns-agent-timeline")).not.toContainText("已完成");
  } finally {
    await electronApp.close();
...
test("streams read tools, restores a question after reload, refreshes dirty context, and links plan execution", async () => {
  ...
  const server = createServer(async (request, response) => {
    ...
    const toolNames = Array.isArray(body.tools)
      ? body.tools
          .map((tool) =>
            typeof tool === "object" && tool !== null && typeof (tool as { function?: { name?: unknown } }).function?.name === "string"
    ...
    if (toolCount === 0) {
      sendToolCall(response, "entries", "list_project_entries", { path: "chapters" }, "先读取项目结构。");
      return;
    }
    if (toolCount === 1) {
      sendToolCall(response, "chapter", "read_chapter", { chapterId: activeChapterId }, "读取当前章节。");
      return;
    }
  ...
  try {
    ...
    await expect(timeline.getByText(`已读取章节 ${activeChapterId}`, { exact: true })).toBeVisible();
...
async function configureLocalModel(page: Page, baseUrl: string): Promise<void> {
  ...
  await expect(page.locator(".ns-project-feedback")).toContainText("Connected to openai-compatible/local-agent");
...
function sendToolCall(
  ...
  response.write(`data: ${JSON.stringify({ choices: [{ delta: { ...(content === undefined ? {} : { content }), tool_calls: [{ index: 0, id: `call_${id}`, type: "function", function: { name, arguments: JSON.stringify(args) } }] } }] })}\n\n`);
